...
const ServerDetailsBlock = ({ className }: { className?: string }) => {
    ...
    return (
        <div className={classNames('grid grid-cols-6 gap-2 md:gap-4', className)}>
            ...
            <StatBlock icon={faMicrochip} title={'CPU-belastning'} color={getBackgroundColor(stats.cpu, limits.cpu)}>
                {status === 'offline' ? (
            ...
            <StatBlock icon={faCloudDownloadAlt} title={'Netværk (indgående)'}>
                {status === 'offline' ? <span className={'text-gray-400'}>Offline</span> : bytesToString(stats.rx)}
            </StatBlock>
            <StatBlock icon={faCloudUploadAlt} title={'Netværk (udgående)'}>
