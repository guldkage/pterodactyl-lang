...
export default () => {
    ...
    return (
        <PageContentBlock title={'SSH-nøgler'}>
            <FlashMessageRender byKey={'account'} />
            <div css={tw`md:flex flex-nowrap my-10`}>
                <ContentBox title={'Tilføj SSH-nøgle'} css={tw`flex-none w-full md:w-1/2`}>
                    <CreateSSHKeyForm />
                </ContentBox>
                <ContentBox title={'SSH Nøgler'} css={tw`flex-1 overflow-hidden mt-8 md:mt-0 md:ml-8`}>
                    <SpinnerOverlay visible={!data && isValidating} />
                    {!data || !data.length ? (
                        <p css={tw`text-center text-sm`}>
                            {!data ? 'Loading...' : 'Der findes ingen SSH-nøgler for denne konto.'}
                        </p>
                    ) : (
                        data.map((key, index) => (
                            <GreyRowBox
