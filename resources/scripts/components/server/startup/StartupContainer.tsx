...
const StartupContainer = () => {
    ...
    return !data ? (
        !error || (error && isValidating) ? (
            ...
            <ServerError title={'Ups!'} message={httpErrorToHuman(error)} onRetry={() => mutate()} />
        )
    ) : (
        <ServerContentBlock title={'Opstart'} showFlashKey={'startup:image'}>
            <div css={tw`md:flex`}>
                <TitledGreyBox title={'Startkommando'} css={tw`flex-1`}>
                    ...
                    {Object.keys(data.dockerImages).length > 1 && !isCustomImage ? (
                        <>
                            ...
                            <p css={tw`text-xs text-neutral-300 mt-2`}>
                                Dette er en avanceret funktion, der giver dig mulighed for at vælge et Docker-billede, der skal bruges, når du kører
                                denne serverinstans. For eksempel, hvis du gerne vil have en Minecraft server til at bruge Java 8, så kan du indstille Java-versionen til 8 i denne menu.
                            </p>
                        </>
                    ) : (
                        <>
                            ...
                            {isCustomImage && (
                                <p css={tw`text-xs text-neutral-300 mt-2`}>
                                    Dette {"servers"} Docker-billede er blevet manuelt indstillet af en administrator og kan ikke
                                    ændres gennem denne brugergrænseflade.
                                </p>
                            )}
                        </>
                    )}
